...
def create_price(product_id, amount_cents, currency="rub"):
    price = stripe.Price.create(
        unit_amount=amount_cents, currency=currency, product=product_id
    )
    return price["id"]
...
def prepare_payment_for_stripe(payment, domain, scheme="https"):
    if not payment.stripe_product_id:
        product_id = create_product(
            name=str(payment.course.title) if payment.course else "Оплата курса"
        )
    ...
    success_url = (
        f"{scheme}://{domain}/payments/stripe/success/?payment_id={payment.id}"
    )
