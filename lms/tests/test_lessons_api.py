...
class LessonAPITests(APITestCase):
    def setUp(self):
        ...
        self.lesson1 = Lesson.objects.create(
            course=self.course, name="Урок 1", owner=self.user1
        )
        self.lesson2 = Lesson.objects.create(
            course=self.course, name="Урок 2", owner=self.user1
        )
    ...
    def test_create_lesson_authenticated(self):
        self.client.force_authenticate(user=self.user1)
        data = {"course": self.course.id, "name": "Новый урок", "owner": self.user1.id}
    ...
    def test_update_lesson_owner(self):
        ...
        response = self.client.patch(
            self.lesson_detail_url(self.lesson1.id), {"name": "Обновленное имя"}
        )
        self.assertIn(
    ...
    def test_update_lesson_other_user_forbidden(self):
        ...
        response = self.client.patch(
            self.lesson_detail_url(self.lesson1.id), {"name": "Попытка обновления"}
        )
        self.assertIn(
