...
class SubscriptionAPITests(APITestCase):
    ...
    def test_unsubscribe_authenticated(self):
        ...
        response = self.client.delete(self.subscribe_url, {'course_id': self.course.id}, format='json')
        if response.status_code not in (status.HTTP_204_NO_CONTENT, status.HTTP_200_OK, status.HTTP_405_METHOD_NOT_ALLOWED):
